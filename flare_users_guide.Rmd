...
`flare_simulation` directory.  The configuration are spread across the files
...
include an additional inflow and outflow that represents the SSS.
...
Importantly, `insitu_obs_fname`, `met_obs_fname`, and `inflow_file1` **requires** two 
...
defined in the temp_oxy_chla_qaqc.R and, potentially, extract_CTD.Rscript that 
...
* `use_future_inflow`: Use forecast inflow vs. observed inflow (if avialable)
  * `TRUE`: Future inflow
...
* `par_names_save`: vector of names of parameter values estimated desired in output and 
  plots
...
* `variable_obsevation_depths`: Use the depth measurment in the exo to convert
  the depths of a fixed thermistor chain.
  * `TRUE`: convert
  * `FALSE`: don't convert
* `ctd_2_exo_chla`: slope and intercept of linear regression that converts chla
  values in the CTD to the exo
...
* `simulate_SSS`: include SSS in simulations with observed drivers (i.e., data
